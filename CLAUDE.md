...
- **Combat panel** (`_setup_combat_panel` / `_render_combat_panel`): a floating `ColorRect`+`RichTextLabel` card on `UI`, shown only while engaged. A pinned **header** rebuilt from stored initiative rolls (`_combat_header`) never scrolls; below it a **current-round section** (`_combat_turn_section`) lists this turn's events (older rounds live only in the top message log). Events are formatted cards pushed via `_push_combat_event` — `_combat_attack_block` for attacks, `_combat_morale_block` for 2d6 nerve tests (so the player sees *why* a monster fled, not just in the log). The panel relocates to the screen corner farthest from the fight (`_update_combat_panel_placement`), snapping on first placement and gliding on later changes; it never jitters because it only moves when the chosen corner changes.
